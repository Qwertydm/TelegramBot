...
#Подключение к боту
bot = telebot.TeleBot('6462997497:AAGoM0hrvZufCQAcnrCg6J3jEDQqiMTpH10')
geolocator = Nominatim(user_agent='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36 Edg/115.0.1901.188')
#Временные данные
users = {}
...
@bot.callback_query_handler(lambda call: call.data in ['back', 'to cart', 'increment', 'decrement'])
def get_user_count(call):
    ...
    if call.data == 'increment':
        ...
        bot.edit_message_text('Ваш товар был добавлен в корзину! Хотите заказать что-то еще?',
                              ...
                              reply_markup=bt.main_menu_buttons())
...
def cart_handler(call):
    ...
    if call.data == 'clear':
        ...
        bot.send_message(5072564063, 'Новый заказ!')
        db.del_cart(user_id)
