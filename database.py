...
def del_cart(user_id):
    ...
    amount = sql.execute('SELECT pr_amount FROM products WHERE pr_name=?;', (pr_name,)).fetchone()[0]
    pr_quantity = sql.execute('SELECT product_quantity FROM cart WHERE user_id=?;', (user_id,)).fetchone()[0]
    sql.execute(f'UPDATE products SET pr_amount={amount + pr_quantity}'f'WHERE pr_name=?;', (pr_name,))
    # Фиксируем изменения
    connection.commit()
